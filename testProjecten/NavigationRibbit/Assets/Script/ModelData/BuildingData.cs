...
public class BuildingData : MonoBehaviour {
	...
	public GameObject GetNextFloor(string floorName)
	{
		for (int i = 0; i < floorList.Count; i++)
		{
			...
			if (floorName == fl.floorName && i >= floorList.Count-1) { 	// last index return first
				return floorList[0]; 
			} else if (floorName == fl.floorName) { 					//if name are equal, return next index
	...
	public GameObject GetConnector(GameObject Node) /* get gameObject of connector of that node floor(Stairs), May have overloading*/
	{
