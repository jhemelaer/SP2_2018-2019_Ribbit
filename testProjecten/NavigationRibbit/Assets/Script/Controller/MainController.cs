...
public class MainController : MonoBehaviour
{
    ...
    void Awake()// if their is already a maincontroller instance, destroy it. If their isn't an instance, create one
    {
        if (instance == null)
        {
            instance = this;
    ...
    /* create started AR when camera detect marker */
    {
        ...
        switch (appState)
        {
            ...
            case AppState.Navigate:         //beginpoint will not null, dest not null in nav
                NavigateIfNotSamePoint();   //new pos not the same as dest
                break;
            default:
    ...
    public void ClearDestinationPoint()// remove destination data 
    {
    ...
    /* check before navigate that two new point aren't the same location  
    return true if two point refer not to the same room and go to navigate*/
    {
	...
	if in same floor, navigate to connector of floor 
	if can't find route, arrow will directly point to destination*/
    {
        ...
        if (building.IsSameFloor(this.beginPoint, this.destinationPoint))// check if the destination is point is on the same floor
        {
    ...
    #region ShowAR
    //function recive prefabType loop all child if met active it and flag as met, if met more than one destroy it
