...
public class DijsktraAlgorithm : MonoBehaviour {
	...
	public bool FindShortestPath(GameObject floorObject, GameObject startNode, GameObject finishNode)// search for the shortest path between two nodes
	{
		...
		while(currentNode != finishNode && (unVisitedList.Count>=0) ) //unVisitedList.Count.CompareTo(0)  ||  (unvisitedLeft > 0)   
		{
			...
			foreach (GameObject adjacentObject in currentMarkerData.adjacentNodeList)
			{
				...
				if(adjacentObject == finishNode) { // if adjacent is final node, set adjacentNode as currentNode and set isFounded as true
					...
					Debug.Log(" - - Break - -");
					break;
				} else if (unVisitedList.Contains(adjacentObject) ) { //node is not visited, Update it
	...
	private GameObject FindGameObjectFromMarkerData(GameObject floorObject, MarkerData markerData)//returns gameObject from markerList
	{
	...
	public void ResetAllVertexData(GameObject floorObject)// Reset attributes of MarkerData
	{
