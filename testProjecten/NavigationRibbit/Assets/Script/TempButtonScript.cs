...
public class TempButtonScript : MonoBehaviour
{
    ...
    private GameObject arcamera, destinationPoint, Floor0;

    // Use this for initialization
    void Start()
    {
        arcamera = GameObject.Find("ARCamera");
        Floor0 = GameObject.Find("Floor0");
    ...
    public void FindPath() //find path from current marker to destination
    {
        IEnumerable<TrackableBehaviour> trackableList = arcamera.GetComponent<CameraFocusController>().GetActiveTrackable();
        foreach (TrackableBehaviour tr in trackableList)
        {
    ...
    {
        gameObject.GetComponent<ToastMessageScript>().showToastOnUiThread("Loooooooooooooooooooooooooooooooooooogn text");
    }
}
