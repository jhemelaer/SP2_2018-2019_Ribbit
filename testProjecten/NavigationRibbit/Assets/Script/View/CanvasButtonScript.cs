...
public class CanvasButtonScript : MonoBehaviour// controls the object of the canvas
{
    ...
    void Start()//fill the atributes with the data of the gameObjects
    {
        ...
        toastMessageScript.showToastOnUiThread("Head to a point, such as a room number sign, to locate your location.", false);
    ...
    public void OnOpenSerch()// when user clicks on the search icon
    {
        page = Page.Search;
    ...
    public void OnCloseSerch()// when user clicks on the close search icon
    {
        page = Page.Main;
        mapPanel.SetActive(false);
    ...
    public void OnOpenMap()// when user clicks on the map icon in the navigationbar
    {
        page = Page.Map;
    ...
    public void OnCloseMap()// when user clicks on the close map icon in the navigationbar
    {
        page = Page.Main;
        mapPanel.SetActive(false);
        searchPanel.SetActive(false);
    ...
    /* show room dialoge box data with map */
    {
        ...
        roomNameTitle.GetComponent<Text>().text = "Room information: " + mkdt.roomName;
        ...
        if (!isDestination)// when the destination is not set
        {
            ...
            if(MainController.instance.beginPoint != null)
            {
                if(mkdt.roomName == MainController.instance.beginPoint.GetComponent<MarkerData>().roomName)
                {
                    ...
                    roomNavigateButton.GetComponent<Button>().onClick.AddListener(delegate {
                        toastMessageScript.showToastOnUiThread("You are already at your chosen destination. Please select another destination.", true);
                    });
                }
            }
        }
    ...
    public void OnTyping()// when user is typing in the searchbar
    {
    ...
    private void ShowAllRoomOf(List<GameObject> searchMarkerList)
    {
        ...
        foreach (GameObject markerob in searchMarkerList)
        {
            ...
            if (!beginColored)
            {
                ...
                {
                    roomButtonText.text = "Source: " + markerdata.roomName;
            ...
            if (!destColored)
            {
                if (MainController.instance.destinationPoint.GetComponent<MarkerData>().roomName == markerdata.roomName)
                {
                    ...
                    roomButtonText.text = "Destination: " + markerdata.roomName;
                    destColored = true;
                }
            }
        }
