...
public class BuildingData : MonoBehaviour
{
    ...
    public GameObject GetNextFloor(string floorName)
    {
        for (int i = 0; i < floorList.Count; i++)
        {
            ...
            if (floorName == fl.floorName && i >= floorList.Count - 1)
            {   // last index return first
                return floorList[0];
            }
            else if (floorName == fl.floorName)
            {                   //name ard equal return next index
    ...
    public GameObject GetPreviousFloor(string floorName)
    {
        for (int i = floorList.Count - 1; i >= 0; i--)
        {
            ...
            {                   //name ard equal return previous index
